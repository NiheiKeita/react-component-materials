...
import { WeeklyAlenderView } from '.';



const meta: Meta<typeof WeeklyAlenderView> = {
  title: 'views/Calender/WeeklyAlenderView',
  component: WeeklyAlenderView,
  tags: ['autodocs'],
}
export default meta
...
export const HelloButton: Story = {
  ...
  render: () => {
    return <WeeklyAlenderView text="Hello"/>
  },
  play: async ({ canvasElement }) => {
    const button = await within(canvasElement).findByTestId('button');
    expect(button).toBeInTheDocument();
